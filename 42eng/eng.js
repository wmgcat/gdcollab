...
let Graphics = {
	...
	'rect': function(x, y, w, h, color, alpha, tp) {
		...
		this.cvs[(tp || 'fill') + 'Style'] = color || '#000';
		this.cvs[(tp || 'fill') + 'Rect'](x, y, w, h);
		if (alpha != undefined) this.cvs.restore();
		return [x, y, w, h];
	},
	'text': function(text, x, y, color, alpha, size, font, tp, align) {
		this.cvs.save()
			...
			if (size) this.cvs.font = Math.floor(size) + 'px ' + (font || 'Arial');
	...
	'gui': {
		...
		'dragWindow': function(id, x, y, w, h, header, content, headcol) {
			...
			if (header != undefined) {
				...
				Graphics.text(header, memory['window.' + id].x + w * .05, memory['window.' + id].y - w * .05, '#fff', 1, h * .08, 'Consolas', 'fill', 'left-middle');
...
function sound_play(name, volume, looping) {
	if (audio[name] && !mute) {
		if (volume != 0) audio[name].volume = volume || audio[name].volume;
		else audio[name].volume = 0;
		audio[name].play();
